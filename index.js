...
async function enviarImagens(sock, jid) { const imgs = [1, 2, 3, 4].map(i => path.join(__dirname, imgs/img${i}.png)) for (const img of imgs) { if (fs.existsSync(img)) { const buffer = fs.readFileSync(img) await sock.sendMessage(jid, { image: buffer }, { quoted: null }) } } }
